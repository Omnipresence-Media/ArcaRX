...
const primarySections = [
  ...
  { label: "Analytics", items: [
    ...
    { title: "Bookings Analytics",   url: "/admin/bookings-analytics", icon: CalendarCheck },
      ...
      <SidebarContent>
        ...
        <div className="mt-1 border-t border-sidebar-border/40 pt-1">
          <button
            ...
            className={`flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-xs transition-colors hover:bg-sidebar-accent/60 ${isInExtra ? "text-[color:var(--teal)]" : "text-sidebar-foreground/50"}`}
          >
            <Zap className="h-4 w-4 shrink-0" />
            {!collapsed && (
              <>
                <span className="flex-1 font-semibold uppercase tracking-[0.14em]">Arca Extra</span>
                {(extraOpen || isInExtra) ? <ChevronDown className="h-3.5 w-3.5" /> : <ChevronRight className="h-3.5 w-3.5" />}
              </>
            )}
          </button>
